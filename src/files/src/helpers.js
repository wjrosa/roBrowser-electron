...
const Imagecreatefrombmpstring = (data) => {
    ...
    const paletteSize  = offset - 54;
    const imagePalette = array();
    if (paletteSize > 0 && bits < 16) {
        const palette = unpack('C'. paletteSize, substr(data, 54, paletteSize));

        for (let i = 1, p = 0; i < paletteSize; i += 4) {
            b = palette[i+0];
            g = palette[i+1];
            r = palette[i+2];
            a = palette[i+3];
    ...
    // Read ImageData
    const skip      = ( 4 - ( (( (bits * width) + 7) >> 3 ) & 3 ) ) % 4;
    const size      = width * height * (bits >> 3) + skip * height;
    const imageData = unpack('C'. size, substr(data, offset, size) );
    ...
    switch (bits) {
        ...
        // 24 bits BMP
        case 24:
            for (let i = 1, y = height - 1; y > -1; y--, i += skip) {
        ...
        case 4:
            for (i = 1, y = height-1; y > -1; y--, i += skip) {
                for (let x = 0; x < width; x += 2, i++) {
                    const byte = imageData[i];
                    imagesetpixel(img, x, y, imagePalette[byte >> 4  ]);
...
module.exports = { Ksort, Imagecreatefrombmpstring }